...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Trigger Entry Notification after 3 seconds of Splash
    const timer = setTimeout(() => {
      const activeNotif = getEntryNotification();
      const currentUser = getUser();
      
      setIsInitialSplash(false); // Remove the deep blue splash

      if (activeNotif && activeNotif.isActive && currentUser) {
          let shouldShow = false;
          if (activeNotif.target === 'all') shouldShow = true;
          else if (activeNotif.target === 'has_coins' && currentUser.coins > 0) shouldShow = true;
          else if (activeNotif.target === 'no_coins' && currentUser.coins === 0) shouldShow = true;
  ...
  // Deep Blue Initial Splash Screen
  if (isInitialSplash) {
      return (
